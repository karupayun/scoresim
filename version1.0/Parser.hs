...
--     en un formato deseable, para luego ser trabajada. También provee
--     una lista de los nombres de los equipos de Latinoamérica.
...
strToProblem [] = Nothing
strToProblem [_,"--"] = Nothing 
strToProblem [xs,ys] = Just (read xs, read ys)
strToProblem [xs,_,ys,_] = Just (read xs, read ys)
...
strToTeam :: Int -> Int -> Int -> Int -> Int -> Int -> [[String]] -> Team
strToTeam cr ct cs cp fp p xs = do let probs = take p $ drop fp xs 
                                   Team {
                                     name = head $ eliminarEspacios $ xs !! ct,
                                     solved = read . head $ eliminarEspacios $ xs !! cs,
                                     penalization = read . head $ eliminarEspacios $ xs !! cp,
                                     problems = map strToProblem $ map eliminarEspacios probs,
                                     latam = Other,
                                     position = cr}

-- teams: Devuelve la lista de los teams, luego de leer información sobre el html.
teams :: IO [Team]
teams = do t <- tablaTeams
           ct <- columnTeams
           cr <- columnRank
           fp <- colFirstProblem
           p <- cantProblems
           cs <- columnSolved
           cp <- columnPenalization
           return $ map (strToTeam cr ct cs cp fp p) t
